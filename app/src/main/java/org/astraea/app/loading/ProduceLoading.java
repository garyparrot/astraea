...
public class ProduceLoading extends Argument {
  ...
  public void run() throws InterruptedException {
    ...
    producer =
        new KafkaProducer<>(
            Map.of(
                ...
                DataUnit.GiB.of(2).measurement(DataUnit.Byte).longValue(),
                ProducerConfig.BATCH_SIZE_CONFIG,
                DataUnit.MiB.of(10).measurement(DataUnit.Byte).intValue()));
    ...
    Supplier<ProducerRecord<Bytes, Bytes>> nextRecord =
        () -> new ProducerRecord<Bytes, Bytes>(topicName, Bytes.wrap(theValue));
    ...
    while (!Thread.currentThread().isInterrupted()) {
      ...
      for (int i = 0; i < (1000 / iterationCount); i++) {
        ...
        for (int j = 0; j < send10Ms; j++) producer.send(nextRecord.get());
