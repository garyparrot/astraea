...
import org.astraea.common.balancer.tweakers.ResourceShuffleTweaker;
 ...
 */
public class ResourceGreedyBalancer implements Balancer {
  ...
  public static final Set<String> ALL_CONFIGS =
      new TreeSet<>(
          Utils.constants(ResourceGreedyBalancer.class, name -> name.endsWith("CONFIG"), String.class));
  ...
  public Optional<Plan> offer(AlgorithmConfig config) {
    ...
    BiFunction<ClusterInfo, ClusterCost, Optional<Plan>> next =
        (currentAllocation, currentCost) ->
            allocationTweaker
                ...
                .peek(a -> System.out.println("Plan: " + a.proposalClusterCost().value() + " " + a.moveCost().overflow()))
                .filter(
                    plan ->
    ...
    MBeanRegister.local()
        ...
        .property("algorithm", ResourceGreedyBalancer.class.getSimpleName())
