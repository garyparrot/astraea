 ...
 */
public class GreedyRestartBalancer implements Balancer {
  ...
  public static final Set<String> ALL_CONFIGS =
      new TreeSet<>(
          Utils.constants(GreedyRestartBalancer.class, name -> name.endsWith("CONFIG"), String.class));
  ...
  public Optional<Plan> offer(AlgorithmConfig config) {
    ...
    final var deadEndCount = new LongAdder();
    ...
    BiFunction<ClusterInfo, ClusterCost, Optional<Plan>> next =
        (currentAllocation, currentCost) ->
            allocationTweaker
                ...
                .takeWhile(ignored -> deadEndCount.sum() < 500)
                .peek(i -> deadEndCount.increment())
                .filter(
                    newAllocation ->
                        config
                            .movementConstraint()
    ...
    MBeanRegister.local()
        ...
        .property("algorithm", GreedyRestartBalancer.class.getSimpleName())
    ...
    var bestSolution = Optional.<Balancer.Plan>empty();
    while (true) {
      currentIteration.add(1);
      currentMinCost.accumulate(currentCost.value());
      var newPlan = next.apply(currentAllocation, currentCost);
      if (newPlan.isEmpty()) {
        if(moreRoom.get() && deadEndCount.sum() >= 500) {
          var bestScore = bestSolution.map(x -> x.proposalClusterCost().value()).orElse(Double.MAX_VALUE);
          var newScore = currentSolution.map(x -> x.proposalClusterCost().value()).orElse(Double.MAX_VALUE);
          if(newScore < bestScore)
            bestSolution = currentSolution;
          // restart
          currentSolution = Optional.empty();
          currentCost = initialCost;
          currentAllocation = currentClusterInfo;
          deadEndCount.reset();
          continue;
        } else {
          break;
